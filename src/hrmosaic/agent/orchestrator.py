...
class ChatOptions(BaseModel):
    ...
    `tools_disabled` is what **this request** asks to withhold, and only that. The process-wide form
    of the same filter, `MCP_TOOLS_DISABLED` (§12.3), is unioned with it in `router.allowed_tools` —
    not defaulted into this field, because an empty list is unprivileged by construction and a
    default would therefore have let any caller drop the operator's filter for their own turn.
    """
