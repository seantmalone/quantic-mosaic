...
def privileged_options_used(options: ChatOptions) -> list[str]:
    ...
    Every field here carries an empty default, so an absent one and an unstated one are the same
    thing. That is also why `MCP_TOOLS_DISABLED` is unioned into the filter in
    `router.allowed_tools` rather than defaulted into `options.tools_disabled`: an empty list asks
    for nothing and is unprivileged, so a caller stating `"tools_disabled": []` would otherwise have
    dropped the operator's filter for their own turn without ever meeting this check.
    """
    used = []
