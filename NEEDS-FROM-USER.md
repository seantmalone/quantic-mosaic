...
- [x] **2b — `MCP_ALLOWED_HOSTS` on the live Render service.** *Optional; the project was complete
      ...
      20:32Z**: an external `initialize` over the public hostname answered **HTTP 200**. A longer
      external session on 2026-09-12 went further — `initialize` 200, `notifications/initialized`,
      `tools/list` returning all nine tools and a real `search_policy_documents` call — and its
      surviving capture is pinned at
      [`docs/evidence/mcp-external-session-2026-09-12.txt`](docs/evidence/mcp-external-session-2026-09-12.txt).
      That transcript is the whole of the external verification pinned for this gate.
