...
flowchart TB
    ...
    LLM["<b>LLM Provider</b> (env-configured)<br/>agent + judge: gemini-3.5-flash-lite (OpenAI-compat)<br/>failover: Groq gpt-oss-120b · optional: Anthropic<br/>stub: scripted, key-free (CI)"]
    UI & DASH --> WEB
    WEB --> ORCH
    ORCH <--> GUARD
    ORCH --> MCPC
...
| 5 | LLM provider abstraction | `ChatModel` protocol with 3 implementations: `OpenAICompatAdapter`, `AnthropicAdapter`, `StubAdapter`, plus an optional `CachedAdapter` wrapper | `openai` 2.54.0, `anthropic` 1.4.0 | One OpenAI-compat adapter covers Gemini / Groq / OpenRouter / Cerebras / OpenAI. The native Anthropic adapter makes the "provider abstraction" claim real against a genuinely non-OpenAI wire shape. Tool-call argument shapes are normalised at the adapter boundary (OpenAI → JSON string, Anthropic → object; always `json.loads`, never string-match). | LiteLLM (an abstraction over our abstraction); single-provider hard-coding |
| 6 | Agent model | `gemini-3.5-flash-lite` via `https://generativelanguage.googleapis.com/v1beta/openai/` | free tier | The only free tier with tool calling on the vendor's own pricing page **and** an OpenAI-compatible endpoint, with generous RPM/RPD and no published daily token cap. The exact model id is confirmed by `scripts/probe_provider.py` before P10, not assumed. | Groq `gpt-oss-120b` as primary (200 K TPD ≈ 10 % of one eval run); OpenRouter `:free` (50 RPD without credit); Anthropic (no free tier) |
| 7 | Judge model | `gemini-3.5-flash-lite` on a separate `JUDGE_API_KEY` when supplied, else the agent key | free tier | A separate key means a judge overrun cannot break an in-flight agent run. `judge_model` is recorded per verdict. | Groq as primary judge (TPD forces a mid-run family switch); same key always (quota contention) |
...
|---|---|---|
| Gemini free-tier RPM / RPD / TPM for `gemini-3.5-flash-lite` | https://aistudio.google.com/rate-limit | P10 step 0 |
...
`tools/list` — the one generated artifact with a test (§21 row 34).
...
calls × (6 s pacing + 5 s provider) ≈ 66 s, and the expected interactive case is limiter-free at 10–20 s. `tests/unit/test_limiter_burst.py` asserts
...
|---|---|
| `OpenAICompatAdapter` | Gemini / Groq / OpenRouter / Cerebras / OpenAI. Tool-call arguments arrive as a JSON **string**; always `json.loads`. Uses `response_format: {type: "json_schema", strict: true}` when a `response_schema` is given, falling back to prompted JSON plus one repair round-trip when the endpoint rejects strict mode. |
| `AnthropicAdapter` | The native wire shape. Tool-call arguments arrive as an **object**. Structured output via a single forced tool. Tested against an httpx `MockTransport` fixture, so the abstraction is verified without spending anything. |
...
Every adapter emits exactly one `llm_call` span per call plus its `llm_messages` rows, with `provider`, `model`, `purpose`, tokens, `ttfb_ms`,
`finish_reason`, `retry_count`, `cache_hit`, `limiter_wait_ms`, `provider_failover` and `structured_output_mode`.

Failover: on repeated 429/5xx the adapter retries with exponential backoff honouring `Retry-After`, then switches to `LLM_FALLBACK_*` (Groq
`openai/gpt-oss-120b`) and records `provider_failover: true` on the span, so the dashboard narrates it as a designed behaviour.

`scripts/probe_provider.py` is P6's acceptance gate when key #1 is available: it lists models on `LLM_BASE_URL` to confirm `LLM_MODEL` exists and
issues one request carrying both `tools` and a strict `response_format`, recording the outcome and date in `CHANGELOG.md`.
...
CREATE TABLE spans (
  ...
  name TEXT NOT NULL,                  -- 'gemini:gemini-3.5-flash-lite' | 'check_pto_balance'
  started_at INTEGER NOT NULL, ended_at INTEGER, duration_ms INTEGER,
...
- **Key-name denylist** — any key matching `(?i)(api[_-]?key|token|secret|password|authorization| cookie|bearer)` has its value replaced with
  `"[REDACTED]"`.
- **Value regexes** — provider key shapes (`sk-…`, `AIza…`, `gsk_…`), JWTs, and long base64 runs.
...
{ "session_id": "9f2c…", "turn_id": "4a71…", "trace_id": "9f2c…", "outcome": "answered",
  ...
  "trace": [
    ...
    {"seq": 8, "kind": "llm_call", "name": "gemini:gemini-3.5-flash-lite", "duration_ms": 2140, "status": "ok",
...
{ "status": "ok",                                    // ok | degraded
  ...
  "llm": {"agent": {"provider": "openai_compat", "model": "gemini-3.5-flash-lite", "configured": true,
                    "last_status": "ok", "last_latency_ms": 640},
...
`judge_agreement_rate` with `judge_agreement_n`.
...
| `LLM_API_KEY` | **REQUIRED for real answers** (not for boot) | — | The one genuinely required credential (Google AI Studio). Its absence is a deferred validation with the degradation contract above |
...
| `LLM_FALLBACK_BASE_URL` / `_MODEL` / `_API_KEY` | – | Groq `openai/gpt-oss-120b`; `LLM_FALLBACK_API_KEY` defaults to `GROQ_API_KEY` | Failover on repeated 429/5xx |
| `JUDGE_PROVIDER` / `_BASE_URL` / `_MODEL` | – | mirrors `LLM_*` | Judge |
| `JUDGE_API_KEY` | – | falls back to `LLM_API_KEY` | Recommended: a second Google project, so a judge overrun cannot stall an agent run |
| `GROQ_API_KEY` | – | — | Groq key: agent/judge failover and the optional cross-family re-judge |
...
Judge = `gemini-3.5-flash-lite` on `JUDGE_API_KEY` when supplied, `temperature=0`, JSON-schema-constrained output, **one repair retry** with the parse
...
**Judged metrics are computed on `baseline` only** — judging all three would roughly triple the judge volume against a free-tier daily cap on the day
results must be produced, and DocRecall, ToolSelection and Workflow (judge-free) are precisely what the two arms move. Page 11's compare tab footnotes
which metrics were judged on which variants. Budget per sweep: ~150 agent calls plus ~260 judge calls on `baseline` ⇒ **~710 calls for all three
variants**, comfortably inside a day at the observed free-tier limits (§3.1).
...
free Actions minutes on a public repo — each with the date observed).
...
| Concern | Control |
|---|---|
| **Secrets** | Read only through `settings.py` from the environment (R1.5). `.env` gitignored; `.env.example` carries no real values; `gitleaks` scans full history in CI; `redact()` scrubs any leaked value from every payload before persistence, including an exact-match sweep over every `os.environ` value whose key ends `_KEY`/`_TOKEN`/`_SECRET`. A `confirmations.token` never appears in a payload or a response body. |
...
**Minimum viable set: one API key (Google AI Studio), a Render account, and a Turso platform token — all three free, none needing a credit card.**
Everything else is an optional upgrade with a documented degradation or an irreducibly human submission step; `NEEDS-FROM-USER.md` states that at the
top and tracks each item with a checkbox. Local embeddings remove the embedding key entirely, and `StubAdapter` means P0–P9 need no credentials.

### 19.1 Required

| # | Item | Why | When | How | If skipped |
|---|---|---|---|---|---|
| 1 | **Google AI Studio API key** → `LLM_API_KEY` | The one genuinely required credential. Powers the agent (`gemini-3.5-flash-lite`) and, by default, the judge, through one OpenAI-compatible endpoint. Free, no card. | Start of **P10**; P0–P9 build and pass CI with `LLM_PROVIDER=stub`. | https://aistudio.google.com/apikey → sign in → "Create API key" → paste to Claude Code | No real answers. Because credential validation is deferred, the app still boots, `/health` is 200 `degraded`, the dashboard is fully browsable, and `/chat` returns 200 with `outcome: "configuration_required"` and the signup link. Substitute: an existing Anthropic key (`LLM_PROVIDER=anthropic`, `LLM_MODEL=claude-haiku-4-5`, ≈$1 for the whole project). |
...
|---|---|---|---|---|
| 8 | **Groq API key** → `GROQ_API_KEY` | Three payoffs: a cross-family re-judge of the 8 reference items (removes the self-preference-bias caveat); a provider-portability latency slice; and a live failover if Gemini 429s during the recorded demo. Free, no card, ~2 minutes. | Before **P10**. | The judge stays in the Gemini family; `design-and-evaluation.md` states the limitation honestly, mitigated by most metric families being judge-free and by the reference-label agreement rate (§13.7). No demo failover. |
| 9 | **A second Google AI Studio key from a different Cloud project** → `JUDGE_API_KEY` | Gemini free limits are per project, so a separate key means a judge overrun cannot consume the agent's budget mid-run. Two minutes, zero code change. | Before **P10**. | Agent and judge share one quota pool. Judged metrics are scoped to `baseline` anyway, so one key is workable — just with a real mid-run stall risk on the day results must be produced. |
| 10 | **Anthropic API key** (~$5 of credit) | Exercises the second, native (non-OpenAI-shaped) adapter against a real endpoint, making the provider-abstraction claim demonstrated rather than only unit-tested. | Before **P10** if wanted. | The `AnthropicAdapter` is fully unit-tested against an httpx `MockTransport`, so the abstraction is real and verified without spending anything — only the live demonstration is lost. |
| 11 | **~20 minutes adjudicating the 8 reference groundedness labels** (§13.7) | Upgrades judge validation from *"independent model subagent, blind"* to *"human-adjudicated"* on the most heavily weighted rubric bullet. Not a key — a review task: read 8 answers with their evidence and mark each claim. | After **P10**'s first real run, before the docs are finalised at P12. | The labels stay model-authored by an independent subagent and are reported and named as such — accurate and defensible, and never described as human. |
...
| **R-5** | **Free-tier quota exhaustion** mid-eval or, worse, live during the recorded demo. | High | Five layers: judged metrics are scoped to the `baseline` variant, bringing a full three-variant sweep to ~710 calls (§13.9); strictly sequential execution behind a token-bucket limiter honouring `Retry-After`, with every observed 429 recorded in `eval_runs.notes`; most metric families need no judge at all; `LLM_FALLBACK_*` → Groq, recorded as `provider_failover` on the span so it narrates as designed behaviour; and live quotas are re-read on the run day (§3.1). The optional `llm_cache` can warm the two demo prompts before recording, narrated honestly via the `cache_hit` badge — but it is off by default and no committed result depends on it. |
| **R-6** | **Judge credibility.** Agent and judge share the Gemini family by default, inviting a self-preference-bias objection on the most heavily weighted rubric bullet. | Medium | Stated openly, with an accurate count: groundedness, citation support, partial match and the clarification check are judge-derived; citation resolvability, DocRecall, tool selection, argument correctness, workflow completion and action safety are **not** — so the majority of metrics, and every safety and behaviour metric, need no judge. Eight items carry reference labels authored by an independent Opus subagent, blind to the judge's verdicts, and the **agreement rate** is published with its n (§13.7). With `GROQ_API_KEY` the same 8 are re-judged cross-family and that agreement rate is published too. `judge_model` is recorded per verdict. |
...
| 11 | **Cohen's κ judge validation** (`evaluation/kappa.py`, cross-family κ as a requirement, the "reference-vs-judge, never human-vs-judge" naming discipline spread across five sections) | With n = 8 the statistic is uninterpretable. An agreement rate with its n stated is more honest and needs no module (§13.7). |
...
Dependencies are strictly forward. **P0–P9 require no secrets** — the one qualification is P6's live-provider probe, which defers to the start of P10
when key #1 is not yet available. Render credentials are not needed until P11. Each phase ends with a green CI run and a commit, and **every definition
