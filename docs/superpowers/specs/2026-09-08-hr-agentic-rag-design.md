...
| `MCP_TOOLS_DISABLED` | – | *(empty)* | The process default for `options.tools_disabled`, read where `ChatOptions` is built; a request that states the field wins. The filter itself is applied per turn in the agent router (`allowed_tools`), on the array of schemas the model is offered — never as a global `remove_tool` on the shared server |
