...
| **Judge** — decompose, groundedness, citation support, gold-fact entailment, clarification check | **Google `gemini-3.5-flash-lite`** (free tier) | `OpenAICompatAdapter` | `JUDGE_PROVIDER=openai_compat` (default), `JUDGE_BASE_URL` (default `https://generativelanguage.googleapis.com/v1beta/openai/`), `JUDGE_MODEL` (default `gemini-3.5-flash-lite`), `JUDGE_API_KEY` | Zero cost, and a **different model family from the agent**, so judge independence holds by construction — no re-judge machinery is needed (§13.7) |
| **Agent failover** on repeated 429 / 5xx / timeouts | **Google `gemini-3.5-flash-lite`** (free) | `OpenAICompatAdapter` | `LLM_FALLBACK_PROVIDER=openai_compat` (default), `LLM_FALLBACK_BASE_URL` (default Gemini), `LLM_FALLBACK_MODEL` (default `gemini-3.5-flash-lite`), `LLM_FALLBACK_API_KEY` | Keeps a live demo alive; recorded as `provider_failover` on the `llm_call` span |
...
and the stdio transport shown in the video) and `## Cost` ($0 Render Hobby, $0 Turso, no paid database, local embeddings, a free-tier judge and failover, and the
...
| 47 | Judge and failover models | **Google `gemini-3.5-flash-lite` on the free tier for both**, each on its own Google AI Studio key from a different Cloud project | A different vendor and family from the agent, so judge independence holds by construction (§13.7) and the failover keeps a live demo alive — both at zero cost and with no quota contention between them |
