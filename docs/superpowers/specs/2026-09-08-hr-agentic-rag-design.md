...
- **A keep-alive cron — *until the cold start had been measured and published*.** 24/7 pinging consumes ~744 of 750 monthly workspace instance-hours,
  and documenting the cold start scores better than hiding it; the ten-minute keep-alive of §14.4 landed on 2026-09-11, after the n=3 table (its
  in-process layer awaiting `KEEP_ALIVE_URL` on the service), and the published figures stay as measured.
...
call (provider error, unparseable body, a daily cap) keeps the answer already written. The price is one extra synthesis call on the minority of turns
...
variable is unset — which, as of 2026-09-11, is its state on the live service: `KEEP_ALIVE_URL` is carried by `render.yaml` for a blueprint apply (P23) but never by the
`Dockerfile`, and has not been set on the live service, so **this layer is shipped and tested but not switched on**, and a visitor still gets the
measured cold start until an operator sets it (one single-key PUT, no rebuild). Nothing above or below claims otherwise. The second layer is
...
instance-hours **once it is switched on** (and that figure is **unchanged** by the second pinger: an awake instance is counted once however many
...
| 49 | Where the keep-alive lives | **In the application, with the GitHub Actions cron as a second layer** — a lifespan task pinging `{KEEP_ALIVE_URL}/health` every `KEEP_ALIVE_INTERVAL_S` (default 600), started only when the variable is set — and as of 2026-09-11 it is carried by `render.yaml` for a blueprint apply but not set on the live service, so the in-process layer is shipped, tested and **off** | Measured, not assumed: GitHub's `schedule:` is best-effort and ran the `*/10` keep-alive **twice in nine hours** on 2026-09-11, leaving the instance asleep at 14:25Z. An in-process loop needs no scheduler, runs whenever the instance is up — exactly when a ping is needed — and reaches Render's edge because the URL is the public one. The cron is kept because it is the only layer that can wake an instance that is already asleep, and it costs nothing extra: the 744-of-750-hour ceiling is a property of being awake, not of how many pingers there are (§14.4) |
