...
- ~~**Token-level streaming.**~~ **Reversed 2026-09-10 (owner sign-off), performance plan §3 W2-E.** The original reasoning — "on 0.1 CPU with a
  3–8 s response it buys cosmetics" — rested on a response time the measured baseline does not have: a p50 turn takes 13.7 s and a p95 turn 35.2 s, and
  the answer appears all at once at the end of it. Streaming the synthesis call moves **time to first visible answer text** by **−3,049 ms p50 /
  −17,161 ms p95** and costs ~+21 ms of client CPU per turn; it also makes `ttfb_ms` a real time-to-first-byte, which read `ttfb_ms == duration_ms` on
  all 185 recorded spans before it. It is **not** a latency lever — 0 ms comes off turn completion — and it is the last thing Wave 2 ships. The span
  rail is unchanged and still the point: `answer_delta` renders **complete blocks** beneath it, provisionally, and is hard-replaced by the finished
  answer (§9.8, §11.3).
...
- **A keep-alive cron — *until the cold start had been measured and published*.** 24/7 pinging consumes ~744 of 750 monthly workspace instance-hours,
  and documenting the cold start scores better than hiding it; the ten-minute keep-alive of §14.4 landed on 2026-09-11, after the n=3 table, and its
  in-process layer was armed on the live service later the same day (14:26Z); the published figures stay as measured.
...
5. Never invent a chunk_id. Cite only ids listed under CITATION COVERAGE below — an envelope without an id may be read, not cited.
6. When a tool result carries an `as_of` date, state it in the answer.

EVIDENCE
{% for c in chunks %}
<document {% if not c.quarantined %}id="{{ c.chunk_id }}" {% endif %}doc="{{ c.doc_id }}" title="{{ c.doc_title }}" section="{{ c.heading_path }}"
...
**Two synthesis-prompt rules changed at P24**, each against a measured casualty of the published run `r_1789086979_baseline`. Rule **5** now
says *cite only ids listed under CITATION COVERAGE*, and a **quarantined envelope is rendered without its `id`**: `inj-001` cited the quarantined
phishing canary — an id that looked exactly like a citable one, correctly absent from CITATION COVERAGE — G2 stripped it under trigger 4 and the
grounded `policy_fact` it was the only support of was destroyed. The lure is still shown, banner and all, because the answer is allowed to *know* about
a quoted attack it must not cite; it simply has nothing to cite it by. Rule **8** becomes checkable rather than aspirational: every document listed
under CITATION COVERAGE that supports any statement in the answer must be cited at least once, and for each listed document the answer does **not**
cite, the model writes `not used: <doc_id>` in `rationale_summary` (`not used: 2 docs` where naming them all would exceed rule 7's 120 characters). The
deterministic half of the same fix — one bounded second call when the answer is still narrower than its evidence — is §7.4's citation-breadth step.
...
| **G2** | `citation_resolvability` | a cited `chunk_id` is unknown, its displayed metadata mismatches the real chunk, the displayed snippet is not a whitespace-normalised substring of the chunk text, **or** the cited chunk is quarantined | strip the citation; if a `policy_fact` block loses all of its citations, drop the block; if all blocks drop, refuse. Verdict `repair`. Resolution reads the **real index** via `core.corpusread.get_chunk`, not the retrieved set — otherwise a plausible id from a prior turn would resolve. **One narrow recovery sits in front of trigger 1 (P24):** an *unknown* id that shares its first 16 characters (`c_` plus 14 of the 16 hex digits) with **exactly one** id in this turn's citable evidence is a transcription slip and resolves to that chunk — `remote-004` cited `c_57b2015388bbf7c60` against an evidence id of `c_57b2015388bbf7d4` and lost a grounded policy fact to the drop. Candidates come only from the turn's own scored, unquarantined evidence, so nothing G1 did not weigh is ever admitted; two candidates, or none, and the citation is stripped exactly as before | `test_g2_citation_resolvability.py`, including an id that exists in the index but was never retrieved, one that exists nowhere, and P24's transcription slip |
...
the two answers of `r_1789086979_baseline` that were narrower than the evidence they were written from: `expenses-002` cited two of the five documents
its synthesis prompt listed and lost `manager-approval-matrix` — the document carrying the approval authority the question asked for — against an end
...
<doc_id>` under rule 8; if the second answer is still narrow, the first one stands. **The repair has to earn the swap**: the second answer replaces the
first only when it is strictly broader in distinct cited documents, G2 refused nothing and dropped no block from it, and it kept every block the reader
already had. Nothing here ever writes a citation the model did not — a fabricated citation is what G2 exists to strip — and every failure of the extra
...
|---|---|---|---|
| **Streamable HTTP, mounted in-process** | `http` (default) | the deployed service — the graded topology | `http://127.0.0.1:${PORT}/mcp-server/mcp`; external clients also need a `Host` on `MCP_ALLOWED_HOSTS` |
...
**`multi_doc` (P24).** `RouteDecision` carries one more boolean: whether a complete answer needs rules from more than one document of the corpus
listed under CORPUS. Nothing routes on it — the catalog gate, the workflows and the confusion matrix are unchanged — and it gates exactly one thing,
§7.4's citation-breadth step, which is allowed to spend a second synthesis call only on a turn the router called multi-document (or a workflow turn,
which is one by construction). The `fallback_decision` of a failed router sets it `false`: a router that could not answer has claimed nothing about the
question's shape, and the extra call is not spent on a guess.
...
class ChatModel(Protocol):
    async def complete(self, messages: list[Message], *, tools: list[ToolSchema] | None = None,
                       ...
                       on_delta: Callable[[str], None] | None = None) -> Completion: ...
```
...
`cache_creation_input_tokens` / `cache_read_input_tokens`), `cost_usd_estimate`, `ttfb_ms`, `streamed`, `finish_reason`, `retry_count`,
`cache_hit`, `limiter_wait_ms`, `provider_failover` and `structured_output_mode`. `ttfb_ms` is **`first_delta_ms or round_trip_ms`** and
`streamed` says which of the two it is: the streaming `AnthropicAdapter` reports the first token, while `OpenAICompatAdapter` — the judge and
the failover path — does not stream and reports the round trip, so §11.6 page 5 never mixes two definitions in one column.
...
|---|---|---|---|---|
| **1** | `/dashboard` · `/api/traces/overview` | `kpis{sessions_24h, sessions_total, turns, tool_calls, guardrail_blocks, escalations, pending_confirmations, error_rate, error_turns, duration_n, p50_ms, p95_ms, tokens_in, tokens_out, est_cost_usd, spend_today_usd, spend_7d_usd, llm_calls_today, llm_daily_call_cap}`, `turns_per_hour[]`, `latest_sessions[{…, auth_mode, actor_role}]`, `health{llm_provider, llm_model, mcp_up, tool_count, doc_count, chunk_count, store_backend, git_sha, uptime_ms, rss_mb, data_as_of}` | — | turns-per-hour sparkline |
...
| **4** | `/dashboard/turns` · `/api/traces/turns` | `rows[{turn_id, session_id, dashboard_url, seq, started_at, user_message, outcome, intent, workflow, duration_ms, llm_calls, tool_calls, retrievals, guardrail_hits}]` | page 2's filter bar plus intent and workflow | — |
...
| **7** | `/dashboard/tools` · `/api/traces/tools` | `by_tool[{tool_name, calls, sample_n, errors, confirmation_pauses, error_rate, duration_n, p50_ms, p95_ms, last_called_at}]`, `recent[{span_id, turn_id, tool_name, arguments, arguments_summary, result_preview, result_summary, is_error, paused_for_confirmation, outcome_label, error_code, duration_ms, actor_employee_id}]` | tool · errors only · date range | — |
...
| **10** | `/dashboard/corpus`, `/dashboard/corpus/{doc_id}` · `/api/corpus/*` | `documents[{doc_id, doc_title, source_format, topics, section_count, chunk_count, word_count, estimated_pages}]`; detail: `document{…, full_text}`, `chunks[{chunk_id, heading_path, char_start, char_end, n_chars, text}]` | topic · format · free text | — |
| **11** | `/dashboard/evals`, `/dashboard/evals/{run_id}` · `/api/eval/{runs,runs/{id},compare}` | list: `runs[{run_id, label, variant, target, git_sha, created_at, n_items, headline{}, judge_model, duration_s, est_cost_usd}]`; detail: `metrics{…}` (below), `verdict{items_scored, items_passed, pass_rate, by_category{}, previous_run_id, previous_created_at, previous_pass_rate}` and `items[{item_id, category, question, gold, answer, scores{}, verdicts{}, latency_ms, cold, passed, turn_id}]`; compare tab: `variants[{variant, metrics{}}]`, `flips[{item_id, baseline_passed, variant_passed}]`, `chunk_size[{chunk_chars, doc_recall_mean}]`; metrics tab: `latency{p50, p90, p95, p99, n_warm, n_cold, cold_p50, cold_p95, by_kind{}}`, `rss_series[]` | run · variant · category · failures-first · cold only | grouped ablation bars · latency histogram · cold-vs-warm bars · RSS line |

**Page 5's `Streamed` column** sits beside `First token` (the `ttfb_ms` field) because the two are read together: on a streamed call `ttfb_ms` is the
time to the first token, on one that did not stream — `OpenAICompatAdapter`, the judge and the failover path — it is the whole round trip (§9.8, W2-E).
Without the flag the column would silently mix the two definitions.

**Display, amended (UX W4).** The routes, the `/api/*` payloads and every field name above are unchanged; what changed is how the pages *say* them.
Nav and page titles are English where the technical noun did not earn its place — page 5 is **Model calls**, page 8 **Guardrails**, page 9
**Tool server**, page 10 **Policy library** — and every page carries a one-line lede, which `_page()` takes as a required keyword so a new page cannot
ship without one. Column labels follow (`auth_mode` → Sign-in, `actor_role` → Role, `TTFB` → First token, `Cache hit` → Cached, `Limiter wait` →
Rate-limit wait, `k` → Passages requested, `k source` → Chosen by, `Max dense` → Best match, `Guardrail hits` → **Guardrail blocks** beside a new
**Guardrail checks** count); the **keys**, the `data-col` / `data-kpi` / `data-metric` hooks and the filter parameter names do not. Every number goes
through one registered Jinja filter (`num`, `ms`, `ms_n`, `secs`, `pct`, `rate`, `score`, `usd`, `ts`, `counted`); a rate over fewer than twenty
samples carries its denominator and a percentile over fewer than five prints `n=`; the `/api/*` payload keeps every figure unrounded. Fields added by
the wave are additive only and are listed in the table above.
...
| `MCP_TOOLS_DISABLED` | – | *(empty)* | Tool names this process withholds on every turn, applied in the agent router (`allowed_tools`) on the array of schemas the model is offered — never as a global `remove_tool` on the shared server. **Unioned** with the per-request `options.tools_disabled`, so a request can withhold more tools and never fewer: an empty `options.tools_disabled` is unprivileged, so a default a request could state its way past would be a hole rather than a knob |
| `MCP_ALLOWED_HOSTS` | – | `127.0.0.1:*,localhost:*` | Comma-separated `Host` values the mounted Streamable HTTP endpoint accepts, passed to the SDK's `TransportSecuritySettings` with `enable_dns_rebinding_protection=True` and `allowed_origins` derived as `http(s)://<host>`. The SDK auto-enables that protection for a loopback bind address, so passing nothing is **not** "no allowlist" — it is the loopback allowlist, and the deployed endpoint answers `421 Invalid Host header` to every external MCP client. `render.yaml` appends `mosaic-hr-copilot.onrender.com` (§8.1) |
...
variable is unset — the state on a laptop and in CI. `KEEP_ALIVE_URL` is carried by `render.yaml` for a blueprint apply (P23) but never by the
`Dockerfile`, and it **is set on the live service**: one single-key PUT on 2026-09-11 at 14:26Z, no rebuild, so **this layer is armed on the live
service since 2026-09-11 14:26Z**, verified by `/health`'s `app.uptime_ms` (60 min at 18:37Z, 69 → 86 min across 23:56Z–00:13Z with no traffic but
two health reads, 124.5 min at 00:51Z on 2026-09-12 — an uptime that crosses the 15-minute idle timer is a self-ping). The measured cold start is
what a visitor gets if the loop is turned off again. Nothing above or below claims otherwise. The second layer is
`.github/workflows/keepalive.yml`, kept rather than replaced. The order was settled by measurement: GitHub's `schedule:` is best-effort and de-prioritises low-traffic repositories, and in the nine hours after the workflow was pushed its `*/10` cron produced
...
instance-hours **while it is switched on**, which it has been since 14:26Z on 2026-09-11 (and that figure is **unchanged** by the second pinger: an awake instance is counted once however many
things ping it), and exhausting them suspends free services until the month resets rather than billing anything. Clearing `KEEP_ALIVE_URL` and
disabling the workflow (one environment variable, one menu, no commit) restores the measured behaviour.
...
| 49 | Where the keep-alive lives | **In the application, with the GitHub Actions cron as a second layer** — a lifespan task pinging `{KEEP_ALIVE_URL}/health` every `KEEP_ALIVE_INTERVAL_S` (default 600), started only when the variable is set — carried by `render.yaml` for a blueprint apply and **set on the live service on 2026-09-11 at 14:26Z**, so the in-process layer is shipped, tested and **armed**, with `/health`'s `app.uptime_ms` past the idle timer as the evidence | Measured, not assumed: GitHub's `schedule:` is best-effort and ran the `*/10` keep-alive **twice in nine hours** on 2026-09-11, leaving the instance asleep at 14:25Z. An in-process loop needs no scheduler, runs whenever the instance is up — exactly when a ping is needed — and reaches Render's edge because the URL is the public one. The cron is kept because it is the only layer that can wake an instance that is already asleep, and it costs nothing extra: the 744-of-750-hour ceiling is a property of being awake, not of how many pingers there are (§14.4) |
