...
def test_only_an_answered_turn_is_announced_as_an_answer_being_ready():
    ...
    assert api.TURN_ANNOUNCEMENTS["refused"] == "I can't answer that one."
...
async def test_the_page_reads_the_sentences_from_the_server_rather_than_keeping_a_second_copy(web):
    ...
    assert "paintStatus(announcementFor(JSON.parse(event.data).outcome), true);" in html
    ...
    assert "paintStatus(announcementFor(lastTurn.dataset.outcome), true);" in html
