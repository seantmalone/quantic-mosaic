...
async def test_a_reminded_turn_takes_another_act_step_instead_of_closing(run_agent, spans, store):
    ...
    assert summary["step_summaries"] == [
        ...
        "step 3: 0 corpus search(es) over 0 document(s), and the question may span more",
...
def test_a_turn_that_searched_once_is_reminded_that_the_corpus_is_federated():
    ...
    assert turn.step_summaries == ["step 0: 1 corpus search(es) over 0 document(s), and the question may span more"]
...
def test_a_turn_that_has_not_searched_at_all_is_not_told_that_it_has():
    ...
    assert turn.step_summaries == ["step 0: 0 corpus search(es) over 0 document(s), and the question may span more"]
