...
And four of them (01, 03, 07, 09) simply **left out** a `not_stated` row: the device requirement and
the written-approval requirement were absent from the answer, so a reader was told their request was
in order on rows nobody had checked.
"""
...
def test_a_mandatory_step_from_the_rules_is_a_cited_policy_fact():
    ...
    assert result.blocks[0]["citations"] == ["c_notice"], "the verdict's own resolved evidence"
...
def test_every_not_stated_row_is_said_in_one_explicit_line():
    result = compliance_restatement.apply([], [envelope()])
    assert result.unchecked == [
        "Written manager approval in MosaicOne: not verified from your record — confirm before you proceed."
    ]
