...
**Tests and coverage.** `make test` runs the whole suite in one command — 2,541 tests as of
2026-09-15, unit, contract, integration, architecture and e2e-with-stub, every one of them against
the scripted stub provider, so no credential is involved. 276 of those are the browser-based UX
...
over 8,243 statements**, which `coverage report` prints as the combined **94%** the gate reads. The
