...
Generated by `evaluation/runner.py` from `evaluation/results/r_1789086979_baseline.json`. Every figure below
comes from that one run; nothing here is hand-edited.

| | |
|---|---|
| Run | `r_1789086979_baseline` |
| Variant | `baseline` |
...
| Judge model | `gemini-3.5-flash-lite` · 249 judge calls · `judge_status: judged` |
...
|---|---|---|---|
| Groundedness (mean, claim-level) | 0.982 | 18 | – |
...
|---|---|---|
| `inj-001` | simple_policy | 1 policy_fact block(s) dropped by G2 |
...
| `remote-004` | tool_task | 1 policy_fact block(s) dropped by G2; workflow completion 0.00 < 1.00 |
...
p50 **16698 ms** · p90 29880 ms · p95 32378 ms · p99 33614 ms (n_cold = 0, cold p50 – ms)

Decomposition across the run: llm_ms 377294 ms · retrieval_ms 22376 ms · store_ms 618 ms · tool_ms 27626 ms.
...
> observed values are baseline **0.846** and no_structured_tools **0.615**
> (delta **-0.231**). Read the table as a measurement, not as evidence that the agentic layer
> does the work.

Items whose strict pass flips against `baseline`: `inj-001` (dense_only_k2), `remote-002` (dense_only_k2), `expenses-002` (dense_only_k2), `onboarding-001` (dense_only_k2), `equipment-001` (dense_only_k2), `remote-004` (dense_only_k2), `unsafe-001` (dense_only_k2), `remote-002` (no_structured_tools), `profile-001` (no_structured_tools), `pto-002` (no_structured_tools), `pto-003` (no_structured_tools), `benefits-002` (no_structured_tools), `unsafe-001` (no_structured_tools)
...
| remote-002 | multi_doc | answered | 0.85 | 1.00 | 1.00 | search_policy_documents | ✅ |
...
| pto-002 | tool_task | answered | 1.00 | 1.00 | 1.00 | check_pto_balance, search_policy_documents, get_policy_section | ✅ |
| pto-003 | tool_task | answered | 0.93 | 1.00 | 1.00 | check_pto_balance, check_policy_compliance, search_policy_documents, lookup_employee_profile | ✅ |
