...
Generated by `evaluation/runner.py` from `evaluation/results/r_1790067656_baseline.json`. Every figure below
comes from that one run; nothing here is hand-edited.

| | |
|---|---|
| Run | `r_1790067656_baseline` |
| Variant | `baseline` |
...
| Judge model | `gemini-3.5-flash-lite` · 252 judge calls · `judge_status: judged` |
| Retrieval | `hybrid_rrf`, k = 5 |
| Guardrail thresholds | MIN_EVIDENCE_SCORE = 0.6 · MIN_SUPPORT_SCORE = 0.45 |
| Limiter | LLM_RPM = 10 |
| Estimated spend | $0.7475 |
| Wall clock | 428.6 s |
| Harness git sha | `e85305b6a764a295a00ef9579c9499623751874e` |
| Target git sha | `e85305b6a764a295a00ef9579c9499623751874e` |
...
| `equipment-001` | multi_doc | groundedness 0.80 < 0.85 |
...
p50 **15345 ms** · p90 23076 ms · p95 24164 ms · p99 25802 ms (n_cold = 0, cold p50 – ms)

Decomposition across the run: llm_ms 315471 ms · retrieval_ms 22240 ms · store_ms 756 ms · tool_ms 26514 ms.
...
judge pass made 252 calls**; a pass runs 249–296 calls and **≈ $0.16–$0.18**,
...
|---|---|---|---|
| `groundedness_mean` | 0.975 | not judged | not judged |
| `citation_accuracy_mean` | 0.871 | not judged | not judged |
| `cit_resolve_mean` | 1.000 | 1.000 | 0.964 |
| `doc_recall_mean` | 0.961 | 0.974 | 0.987 |
| `tool_selection_accuracy` | 0.988 | 0.988 | 0.924 |
| `arg_correctness_rate` | 1.000 | 1.000 | 1.000 |
| `workflow_completion` | 0.964 | 0.964 | 0.786 |
| `over_refusal_rate` | 0.000 | 0.000 | 0.000 |
| `strict_pass_rate` | 0.893 | 0.929 | 0.750 |
...
> observed values are baseline **0.964** and no_structured_tools **0.786**
> (delta **-0.179**). Read the table as a measurement, not as evidence that the agentic layer
> does the work.

Items whose strict pass flips against `baseline`: `equipment-001` (dense_only_k2), `remote-002` (no_structured_tools), `equipment-001` (no_structured_tools), `profile-001` (no_structured_tools), `pto-002` (no_structured_tools), `pto-003` (no_structured_tools), `benefits-002` (no_structured_tools), `amb-003` (no_structured_tools), `unsafe-001` (no_structured_tools)

All three runs share `target: deployed`, `dataset_sha: e83cc9fc4833e548…` and `target_git_sha: e85305b6a764…`, which `evaluation/ablation.py` asserts before it writes anything. Judged metrics are computed on `baseline` only (§13.9); a `null` on an ablation arm means not judged, never zero.
<!-- ABLATION:END -->
...
| expenses-002 | multi_doc | answered | 1.00 | 1.00 | 1.00 | search_policy_documents, lookup_employee_profile, check_policy_compliance | ✅ |
| onboarding-001 | multi_doc | answered | 1.00 | 1.00 | 1.00 | search_policy_documents | ✅ |
| equipment-001 | multi_doc | answered | 0.80 | 1.00 | 1.00 | search_policy_documents | ❌ |
...
| remote-004 | tool_task | answered | 0.88 | 1.00 | 0.75 | lookup_employee_profile, check_policy_compliance, search_policy_documents | ❌ |
...
G5 wave: published run on build e85305b config.llm_rpm is this harness's rate, not the target's: the run was driven against a remote service whose own LLM_RPM/LLM_BURST are set in its environment (effective target rate: unknown; service env). The latency in this run was produced under the target's rate (§9.4). Judged in a second pass on 2026-09-22 (252 judge calls, model gemini-3.5-flash-lite); the 28 answers are the drive pass's own and were not re-driven.
