...
Generated by `evaluation/runner.py` from `evaluation/results/r_1790074972_baseline.json`. Every figure below
comes from that one run; nothing here is hand-edited.

| | |
|---|---|
| Run | `r_1790074972_baseline` |
| Variant | `baseline` |
...
| Judge model | `gemini-3.5-flash-lite` · 268 judge calls · `judge_status: judged` |
| Retrieval | `hybrid_rrf`, k = 5 |
| Guardrail thresholds | MIN_EVIDENCE_SCORE = 0.6 · MIN_SUPPORT_SCORE = 0.45 |
| Limiter | LLM_RPM = 10 |
| Estimated spend | $0.7731 |
| Wall clock | 439.9 s |
| Harness git sha | `8a8931076bac9d271f8a03da7ecfa0d3a723d811` |
| Target git sha | `8a8931076bac9d271f8a03da7ecfa0d3a723d811` |
...
* **nudge_rate** = 0.571 — the share of scored turns whose `plan` span carries a
  non-empty `nudges[]`
...
* **workflow completion by workflow** — `pto_request` 1.00 (n = 1, `pto-003`) · `remote_work_eligibility` 1.00 (n = 1, `remote-004`)
  Each demo workflow is mirrored by exactly one tagged item in `evaluation/dataset.yaml` (§13.1), so these are **single-item indicators, not rates**.

## Latency

p50 **15314 ms** · p90 24146 ms · p95 26035 ms · p99 27457 ms (n_cold = 0, cold p50 – ms)

Decomposition across the run: llm_ms 331198 ms · retrieval_ms 21353 ms · store_ms 910 ms · tool_ms 27551 ms.
...
judge pass made 268 calls**; a pass runs 249–296 calls and **≈ $0.16–$0.18**,
...
Protocol: labeller `blind-opus-labeller — a separate Claude Opus 5 session dispatched by the controller: the same vendor as the agent (Anthropic claude-haiku-4-5), a different model, in an independent session that read only the packet — and a different vendor and family from the judge (Google gemini-3.5-flash-lite)`, labelled 2026-09-22. Authored on 2026-09-22 against the final published run `r_1790074972_baseline` (deployed commit 8a89310). That is the only run in this protocol, and the packet carried that run's own served answers: for each item, the question, the answer `r_1790074972_baseline` served, and — verbatim — every evidence item the synthesis prompt carried for that turn, each labelled with its class. The session was fresh and read only that packet. The packet is judge-free by construction of its builder: `scripts/gen_label_packet.py` reads the dataset, the run file's `item_id` → `turn_id` mapping and served answer, and the trace store's spans, and it never reads `scores` or `verdicts` — so no verdict, no per-claim verdict, no rationale and no groundedness score was in front of the labeller. The judged run file was not written until 04:37:44Z on 2026-09-22, after this packet was built at 04:25:16Z. The session read no run file, no REPORT.md, no CHANGELOG and no phase report.
...
> observed values are baseline **0.964** and no_structured_tools **0.821**
> (delta **-0.143**). Read the table as a measurement, not as evidence that the agentic layer
> does the work.

Items whose strict pass flips against `baseline`: `expenses-002` (dense_only_k2), `equipment-001` (dense_only_k2), `remote-004` (dense_only_k2), `remote-002` (no_structured_tools), `expenses-002` (no_structured_tools), `equipment-001` (no_structured_tools), `profile-001` (no_structured_tools), `pto-002` (no_structured_tools), `pto-003` (no_structured_tools), `remote-004` (no_structured_tools), `benefits-002` (no_structured_tools), `unsafe-001` (no_structured_tools)

All three runs share `target: deployed`, `dataset_sha: e83cc9fc4833e548…` and `target_git_sha: 8a8931076bac…`, which `evaluation/ablation.py` asserts before it writes anything. Judged metrics are computed on `baseline` only (§13.9); a `null` on an ablation arm means not judged, never zero.
<!-- ABLATION:END -->
...
| leave-001 | simple_policy | answered | 1.00 | 1.00 | 1.00 | search_policy_documents, list_policy_documents | ✅ |
| travel-001 | simple_policy | answered | 0.92 | 1.00 | 1.00 | search_policy_documents | ✅ |
| remote-002 | multi_doc | answered | 1.00 | 1.00 | 0.50 | search_policy_documents | ❌ |
...
| equipment-001 | multi_doc | answered | 0.69 | 1.00 | 1.00 | search_policy_documents | ❌ |
...
| pto-002 | tool_task | answered | 0.95 | 1.00 | 1.00 | check_pto_balance, search_policy_documents, lookup_employee_profile | ✅ |
...
| remote-004 | tool_task | answered | 1.00 | 1.00 | 0.75 | lookup_employee_profile, search_policy_documents, get_policy_section, check_policy_compliance | ✅ |
| benefits-002 | tool_task | answered | 1.00 | 1.00 | 1.00 | lookup_benefits_status, search_policy_documents, list_policy_documents | ✅ |
...
G5 wave: published run on the final build 8a89310 config.llm_rpm is this harness's rate, not the target's: the run was driven against a remote service whose own LLM_RPM/LLM_BURST are set in its environment (effective target rate: unknown; service env). The latency in this run was produced under the target's rate (§9.4). Judged in a second pass on 2026-09-22 (268 judge calls, model gemini-3.5-flash-lite); the 28 answers are the drive pass's own and were not re-driven.
judge_agreement_rate=1.0 over n=8 reference labels (subset seed_1729_8).
judge_agreement_rate_hard=0.875 over n=8 reference labels (subset judge_lowest_8). disagreements: expenses-002 (reference grounded, judge not_grounded)
