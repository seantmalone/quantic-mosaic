...
Generated by `evaluation/runner.py` from `evaluation/results/r_1789555212_baseline.json`. Every figure below
comes from that one run; nothing here is hand-edited.

| | |
|---|---|
| Run | `r_1789555212_baseline` |
| Variant | `baseline` |
...
| Judge model | `gemini-3.5-flash-lite` · 291 judge calls · `judge_status: judged` |
| Retrieval | `hybrid_rrf`, k = 5 |
| Guardrail thresholds | MIN_EVIDENCE_SCORE = 0.6 · MIN_SUPPORT_SCORE = 0.45 |
| Limiter | LLM_RPM = 10 |
| Estimated spend | $0.7210 |
| Wall clock | 555.6 s |
| Harness git sha | `bd4ac9336e87f1233f848ef4e5635c29e07cd15b` |
| Target git sha | `bd4ac9336e87f1233f848ef4e5635c29e07cd15b` |

## Headline metrics

| Metric | Value | n | Target |
|---|---|---|---|
| Groundedness (mean, claim-level) | 0.975 | 18 | – |
| Citation accuracy (CitResolve × F1) | 0.883 | 18 | – |
| Citation resolvability (served answer) | 1.000 | 28 | – |
| Document recall | 0.921 | 19 | – |
| Partial match (gold facts entailed) | 0.796 | 18 | – |
| Tool selection (F1, order-insensitive) | 0.981 | 28 | – |
| Argument correctness | 1.000 | 19 | – |
| Workflow completion | 0.964 | 28 | – |
| Action safety pass rate | 1.000 | 1 | – |
| Clarification accuracy | 0.333 | 3 | – |
...
| `remote-004` | tool_task | groundedness 0.80 < 0.85 |
...
p50 **19202 ms** · p90 31509 ms · p95 35707 ms · p99 44341 ms (n_cold = 0, cold p50 – ms)

Decomposition across the run: llm_ms 434141 ms · retrieval_ms 21524 ms · store_ms 866 ms · tool_ms 28384 ms.
...
judge pass made 291 calls**; a pass runs 249–296 calls and **≈ $0.16–$0.18**,
...
Protocol: labeller `blind-opus-labeller — a separate Claude Opus 5 session dispatched by the controller: the same vendor as the agent (Anthropic claude-haiku-4-5), a different model, in an independent session that read only the packet — and a different vendor and family from the judge (Google gemini-3.5-flash-lite)`, labelled 2026-09-16. Authored on 2026-09-11 for the final published run `r_1789166880_baseline` (deployed commit 34717b5) in a fresh session that read only the labelling packet: for each item, the question, the answer the agent served, and — verbatim — every evidence item the synthesis prompt carried, each labelled with its class. The packet carried no judge output of any kind — no verdict, no per-claim verdict, no rationale, no groundedness score — and it did not disclose the selection criterion or the ordering, so the labeller could not tell a low-scoring item from a high-scoring one. The session read no run file, no REPORT.md, no CHANGELOG, no phase report and not the first subset's labels either.

| reference ↓ / judge → | grounded | not_grounded |
|---|---|---|
| grounded | benefits-001, benefits-002, conduct-001, equipment-001, expenses-001, expenses-002, travel-001 | remote-004 |
| not_grounded | – | – |
...
| travel-001 | simple_policy | answered | 0.90 | 1.00 | 1.00 | search_policy_documents | ✅ |
| remote-002 | multi_doc | answered | 1.00 | 1.00 | 0.75 | search_policy_documents | ✅ |
| expenses-002 | multi_doc | answered | 0.85 | 1.00 | 1.00 | search_policy_documents, lookup_employee_profile, check_policy_compliance | ✅ |
...
| remote-004 | tool_task | answered | 0.80 | 1.00 | 0.75 | lookup_employee_profile, search_policy_documents, get_policy_section, check_policy_compliance | ❌ |
