...
- 2026-09-11 — Cold start, measured three times: the queued probes landed and the published figure is now n=3. Probe 2 (02:19Z) and probe 3 (02:37Z) ran on `da0dca2`, the build that served the published evaluation run, each after 1,000 s of deliberate idle and with no keep-alive on the service: 43.5 / 52.4 s spin-up → `/health`, 0.1 / 0.1 s on to `/ready`, 23.9 / 25.2 s for the first `POST /chat`, **67.5 / 77.6 s** cold to first answer, 22.5 / 23.9 s warm. With probe 1 (`bf85ffd`, 2026-09-10) that is a median **71.0 s** cold to first answer over a 67.5–77.6 s range and **22.5 s** warm, which supersedes the `n=1` line above everywhere it was published — `deployed.md`, `README.md`, `NEEDS-FROM-USER.md`, `design-and-evaluation.md`, `docs/requirements-traceability.md` and `docs/architecture.html`. The raw segments, with each probe's timestamp and sha, are committed as `docs/evidence/cold-start-probes.json`. Sean's ruling of 2026-09-10 20:40Z — publish the no-ping measurement, then add a GitHub Actions keep-alive pinging `/health` every ten minutes (≈ 744 of the 750 free instance-hours a month; exhausting them suspends the service rather than billing) — is now stated in `design-and-evaluation.md` and `docs/optimization-log.md` instead of being contradicted there. **The keep-alive workflow itself is not in the repository:** `.github/workflows/` holds `ci.yml` only, and every published cold-start figure is the behaviour with nothing pinging the service. *(Superseded by the entry below: `keepalive.yml` landed two commits later at `3d38ae5`. The cold-start figures are unchanged — they were measured before it, which was the point of the ordering.)*

- 2026-09-11 — Keep-alive landed, **after** the cold start was measured and published. `.github/workflows/keepalive.yml` pings `GET $DEPLOY_URL/health` every ten minutes (`schedule` plus `workflow_dispatch`) on one `ubuntu-latest` job with `permissions: {}` and no secret, because `/health` is an open route; the URL comes from the repository *variable* `DEPLOY_URL` and falls back to the committed live origin, which is what actually resolves today (`DEPLOY_URL` exists as a repository **secret** for CI's `deploy` job, not as a variable). The ping prints `status`, `app.cold_start` and `app.uptime_ms`, and it can never fail the repository's status: any transient error is a `::warning::`, an exit 0 and one line in the job summary, so a suspended free tier is visible in the Actions tab without a red `main`. **The cost is the point of the ordering:** an always-on free instance consumes ~744 of the workspace's 750 monthly instance-hours (24 × 31), and exhausting them suspends free services until the month resets rather than billing anything — so `deployed.md`'s n=3 table stays published exactly as measured, labelled *Measured without keep-alive*, and disabling the workflow (one Actions menu, no commit) restores that behaviour. `scripts/check_render_hours.py` keeps its 600-of-750 warning; reaching ~744 is now the expected shape of a full month, not a leak.

- 2026-09-11 — P22, a confirmed write is reported as done: the live demo-2 turn on `753596e` consumed its confirmation, created `MOCK-HR-000002` in `hr-timeoff` and carried that result verbatim into the synthesis prompt — and the answer still closed with *"I cannot open PTO requests on your behalf. You must submit the request directly in MosaicOne…"*, never naming the ticket. Three changes. **`agent/outcome.py`** is a new post-synthesis step, `outcome_consistency`, with a name rather than a G-number and no `guardrail` span: it reads the turn's write envelopes (`create_mock_hr_ticket` → `status: created`, `draft_hr_email` → `drafted`, neither returnable without a consumed one-time token), opens the answer with a `recommendation` block stating the outcome and the id **built from the tool result**, skips it when the model already stated that id, and replaces an `escalation` that denies the very action the result shows was performed with a line pointing at what was created — denial phrase *plus* a word for what the tool does, so G5's People Operations escalation is never collateral. The step covers **`next_steps` as well as the blocks** (fix round, 2026-09-11): the same answer closed with *"Log into MosaicOne and submit your PTO request for 15–17 September 2026"* one line under the block naming the ticket, so a next step whose clause opens with an imperative for the performed tool and names that tool's own object (`ticket`, `request`, `case`) is dropped — "Watch for your manager's approval" and "Your manager will receive the request" are not directives at the reader and survive. **`synthesize.j2` rule 10** tells the model the same thing (the system half moved, so the golden was regenerated; `act.j2` is byte-identical). **The rail** no longer paints the gated `create_mock_hr_ticket` attempt red: `narration.tone_for()` maps the `CONFIRMATION_REQUIRED` shape to a `pending` tone and the label "Needs your confirmation", the SSE `span` frame carries `tone` beside the untouched `status`, and `.span-pending` is amber — the recorded status stays `error`, because the MCP result is `isError` and the audit record must keep saying so. Also fixed here, because it blocked the deploy: `test_health_reports_every_block_of_the_payload` raced the lifespan's boot import of `evaluation/results/*.json` and failed in CI with `assert 1 == 12` under the coverage tracer while passing locally — `tests/conftest.py::health_after_the_boot_import` now polls `/health` for at most 30 s in 0.2 s steps before the same equality assertion.

- 2026-09-11 — P23, the checkable defects an independent grade card found (1,962 tests): the deployed MCP mount had been answering `421 Invalid Host header` to every external client — the SDK's `streamable_http_app()` defaults `host="127.0.0.1"` and therefore auto-enables the loopback allowlist, so "passing nothing" was never "no allowlist" — now `MCP_ALLOWED_HOSTS` is a setting, `render.yaml` carries the deployment's hostname and `tests/contract/test_mcp_host_allowlist.py` drives real `initialize` requests through the mount for all three cases; the runner records a run's **harness** and **target** shas instead of `git_sha: "dev"`; the blind labeller is described as the same vendor as the agent rather than "a third model family", with a docs test forbidding the old phrase; and the stale test and statement counts, the 120- versus 420-person headcount contradiction, a stale traceability row, the web-font load on the architecture page and the git-ignored audit-trail pointer were all corrected, with the SDD process trail committed under `docs/process/sdd/` and the two live demo transcripts under `docs/evidence/`.

- 2026-09-11 — P24, the model-behaviour wave the grade card's remaining items needed (1,999 tests), measured by re-driving all three deployed arms: multi-document answers now cite every document their evidence spans, with one bounded repair call when the first answer does not; the two `policy_fact` blocks the citation guardrail had dropped were traced to a quarantined chunk that still carried a citable id and to a one-character transcription slip, both fixed at the root; and two HR-adjacent out-of-corpus questions (`oos-004` tuition reimbursement, `oos-005` referral bonus) joined `evaluation/dataset.yaml`, which is now **28 items**, both refused by the router as gold expects. **Measured on `r_1789166880_baseline` (deployed `34717b5`, judged, 263 judge calls): strict pass 0.808 → 0.893** — the project's own ≥ 0.85 target met for the first time — workflow completion 0.846 → 0.893, `blocks_dropped_by_g2` 2 → 0, tool selection 0.993, refusals 0 / 0, groundedness 0.984, citation accuracy 0.905, doc recall 0.961. The price is the repair call's latency: p50 16.7 s → 19.1 s, p95 32.4 s → 38.7 s. Blind reference labels were re-authored for these answers before the judge ran and both subsets agree at **0.875 (n = 8)**, disagreeing on the same item — `expenses-001`, whose next step names a month-end deadline no evidence states. The ablation delta is **−0.143** against the pre-registered 0.25 bar, still **not supported**. Three items still fail the composite: `remote-003` (declines a gold tool its end state also needs), `remote-004` (cited two documents against a three-document end state) and `unsafe-001` (answered instead of stopping at the confirmation card).

- 2026-09-11 — P25, the publish pass: the graded documents now carry the final run. `README.md`, `design-and-evaluation.md` (results block regenerated by `scripts/paste_eval_numbers.py`, plus the hand-written strict-pass causes, ablation, agreement and limitations), `docs/requirements-traceability.md`, `deployed.md`, `ai-tooling.md`, `docs/demo-script.md` and `docs/architecture.html` all state `r_1789166880_baseline`'s figures over 28 items, with the **four** measured columns published side by side and the last column's changed item count disclosed rather than averaged away. **RUBRIC5.1 is now verified on all three of its thresholds** (groundedness 0.984 ≥ 0.90, `cit_resolve_mean` 1.000 ≥ 0.95, strict pass 0.893 ≥ 0.85). The MCP wording follows the live endpoint: `MCP_ALLOWED_HOSTS` is set on the service and an external `initialize` over the public hostname answered **HTTP 200 on 2026-09-11 at 20:32Z** — that one `initialize` is the extent of the external verification, and the bearer and confirmation gates stay asserted by the test suite — so `mcp/README.md`, `deployed.md`, `design-and-evaluation.md` and `docs/architecture.html` say the public mount accepts external clients, and keep `/dashboard/mcp` and `mcp/run_stdio.sh` as the other two ways to see the same nine tools. `NEEDS-FROM-USER.md`'s gate **2b** is discharged — `MCP_ALLOWED_HOSTS` is set on the live service, and the keep-alive variable that used to sit beside it is recorded in `deployed.md` as the reversible operator preference it is, since every published cold-start figure is measured without it — so only the demo video (gate 6) and the submission (gate 7) remain open.

- 2026-09-11 — P26, the keep-alive is armed and the documents say so. `KEEP_ALIVE_URL=https://mosaic-hr-copilot.onrender.com` and `KEEP_ALIVE_INTERVAL_S=600` were set on the live service at **14:26Z** with a single-key PUT — no rebuild, and `render.yaml` has carried the same pair since P23 so a blueprint apply cannot undo it — and `/health`'s `app.uptime_ms` is the evidence that the in-process self-ping works: 60 min at 18:37Z, 69 min at 23:56Z → 86 min at 00:13Z across a seventeen-minute window whose only other traffic was two health reads, and 124.5 min at 00:51Z on 2026-09-12, all past Render's 15-minute spin-down. `README.md`, `deployed.md`, `design-and-evaluation.md`, `docs/architecture.html`, `docs/demo-script.md`, the design spec's §14.4 / R-10 / decision 49 and `NEEDS-FROM-USER.md` now publish that state with its date and evidence; the n=3 cold-start table stays exactly as measured, relabelled as what a visitor gets if the loop is ever turned off, with the two menus that turn it off unchanged. `tests/contract/test_keep_alive.py` flips with it: `CONDITIONAL_MARKERS` becomes `ARMED_MARKERS` plus a `STALE_MARKERS` ban, so a document that still says the layer is off fails, while the invariants that `render.yaml` carries the origin and the `Dockerfile` never bakes it are untouched. Two P25 minors ride along: the four-column table's column-2 judge-agreement cell reads "not published (labels are authored per published run)" and discloses the run file's mechanically computed 1.00 (n=8) against labels authored for other answers, and `docs/pre-submission-checklist.md`'s **SUB.2** names the half still to be checked on the day, so it agrees with the traceability matrix's `verified`.

- 2026-09-11 — P27, the residuals the re-grade left (2,002 tests). The commit census in `ai-tooling.md`'s academic-integrity disclosure is recomputed from `git log` — **160 commits through `5b1bd51`: 122 `Claude Opus 5 (1M context)`, 35 `Claude Fable 5.1`, 3 branch merges** — and `tests/contract/test_docs_completeness.py` now recounts those four figures from the history **at the commit the paragraph names**, so the hand-typed count that had drifted by fourteen commits cannot drift silently again (anchoring the assertion on `HEAD` instead would falsify a true sentence with the very next commit). The process trail under `docs/process/sdd/` gains P24–P26's briefs and reports, P27's brief and the current ledger, each scanned for secret shapes before copying exactly as at P23 and adding no match; its README states the coverage and the one-report lag the copy order creates. `docs/requirements-traceability.md`'s summary sentence stops counting **SUB.3** among the rows still waiting on a human, now that its own row reads `done` — 72 `built` + 4 `verified` + 1 `done` + 10 `planned` = the 87. The MCP wording in `mcp/README.md`, `deployed.md` and `design-and-evaluation.md` now states the **whole** verified session of 2026-09-11 rather than its first request — `initialize` 200, `notifications/initialized` 202, `tools/list` returning all nine tools, real `search_policy_documents` and `check_pto_balance` calls, `create_mock_hr_ticket` refused `CONFIRMATION_REQUIRED` with no confirmation token and again with a forged one, and 401 with no bearer — which supersedes the P25 entry above, where that one `initialize` was said to be the extent of the external verification. And demo task 2, re-run live on the final build (`/health` sha `f5e86c3`, 2026-09-12 01:39Z), is pinned as `docs/evidence/demo-task-2-live-2026-09-12.txt` — the gate held, `MOCK-HR-000006`, 4 citations across 2 documents, 40 s — linked from `README.md`, the **R6.5** traceability row and `docs/demo-script.md`'s ⑤ beat, which records that this answer named the ticket id itself and so did not get the deterministic *"Done: …"* opener.

- 2026-09-11 — P27 fix round 1 (2,003 tests): the two guards the re-grade's residuals fix had left unenforceable. **The commit census guard never ran in CI.** The `test` job checked out at `actions/checkout`'s default `fetch-depth: 1`, so the anchor commit `5b1bd51` was not in the clone and `test_the_commit_census_is_the_one_git_log_reports` skipped on every run — a skip that reads as a pass. The `test` job now checks out at `fetch-depth: 0` like `lint`, and the test's skip is *earned*: it is reachable only from a checkout that could not hold the commit (no `git`, or `git rev-parse --is-shallow-repository` is true), and a full clone that cannot resolve the sha **fails** instead. (The `paths-ignore` filter still means an edit to a repo-root `*.md` alone triggers no run at all; that filter is ratified in `constraints.md` to match spec §15.1 and is left as it is — the guard now runs on every push that does trigger CI.) **The external MCP session is pinned, not asserted.** `mcp/README.md`, `deployed.md` and `design-and-evaluation.md` stated a complete external session — nine tools, both refusals, 401 with no bearer — with no artefact in the repository behind any of it, which is the one graded claim in this build that rested on prose. `docs/evidence/mcp-external-session-2026-09-12.txt` now pins what survives as a capture: `initialize` **200** with a `mcp-session-id` through Cloudflare from Render's uvicorn, `notifications/initialized`, `tools/list` returning all **nine** tools with their full published schemas, and a real `search_policy_documents` call answered out of the deployed index with its server-side retrieval span (hybrid RRF over 32 candidates, `index_version` 2026.1+920c). The four exchanges that were **not** captured — `check_pto_balance`, `create_mock_hr_ticket` refused `CONFIRMATION_REQUIRED` untokened and again with a forged token, and 401 with no bearer — are now attributed to the 2026-09-12 01:45Z re-grade record rather than stated flatly, in all three documents, in the transcript's own header and in `NEEDS-FROM-USER.md`'s gate 2b; both gates remain held by the suite on every run. `docs/architecture.html`'s `f1-inspector` node, which still carried P25's superseded *"that one initialize is the whole of the external verification"* and therefore contradicted its three siblings on the exact claim, now carries the same summary. `tests/contract/test_docs_completeness.py` gains `test_the_external_mcp_session_is_pinned_and_every_document_that_claims_it_cites_it`, so a document may publish the claim only while it cites the artefact.

- 2026-09-21 — **G5, the grade-and-fix wave**, and what the grade found. An independent read-only grading pass against `docs/project-requirements.md` — 82 agents over grouped rubric sections, one adversarial skeptic per flagged finding, one synthesising grader — read the repository and the live service at `98c893f` and returned **band 4, at the top of the band**, with the built system called band-5 work and the cap laid entirely on the publish pass: `evaluation/results/latest.json` and `evaluation/REPORT.md` published `r_1789555212_baseline` while `README.md`, `design-and-evaluation.md`, `deployed.md`, `docs/requirements-traceability.md`, `docs/pre-submission-checklist.md` and `docs/demo-script.md` all published `r_1789166880_baseline` — eight headline figures different, four of them flattering the submission, under a block whose own footer says it is written from `latest.json` and must not be hand-edited, with `python scripts/paste_eval_numbers.py --check` exiting 1 the whole time. Secondary caps: clarification accuracy 0.333 published nowhere but `REPORT.md`, a *Known limitations* list describing a superseded run's failures, and a documented local-run path a fresh clone cannot execute because `make ingest` is named in no document and the index is untracked. The card is committed verbatim as `docs/evidence/grade-card-2026-09-21.md` with its ranked gap list as `docs/evidence/grade-card-2026-09-21-gaps.json`, and the plan that mapped every gap to a task is `docs/superpowers/plans/2026-09-21-grade-5.md` (`82994ce`).

- 2026-09-22 — **G5, what each task changed.** **Task 1** (`2557d0c`, fix round `e4007f1`): a clarification question names **every** missing slot rather than the first, the profile debt keys on the arguments a tool was actually called with, a refused confirmation says so instead of rendering under *"You approved this — it went ahead"*, and the router sees its own tool catalog. **Task 1b** (`5b66ee6`, fix round `e85305b`): when the router names no workflow, the clarification infers the slot order from the topic of its own rationale, which is what took clarification accuracy off 0.333 — and the inference fires **only** when `workflow is None`, so a named workflow is never second-guessed. **Task 1c** (`54a2895`, fix rounds `dcc5d9c`, `8a89310`): a confirmed `draft_hr_email` is narrated as done rather than refused for lack of retrieval evidence (guardrail G1 gained `grounded_by_write`), and a cancelled or failed write is answered by its receipt — never above a decision line claiming it went ahead. **Task 2** (`b0261d6`): `evaluation/ablation.py` now asserts the compared arms share `target_git_sha` as well as `target` and `dataset_sha` — an arm measured on one commit against an arm measured on the next is a build artefact, not an ablation — the compare tab renders the hypothesis metrics, and a cold cell with no probes says it was not measured instead of printing zero. **Task 3** (`7678e0c`, fix rounds `e923bc3`, `ef917a3`): `MCP_TOOLS_DISABLED` is read rather than ignored and the process filter is **additive**, so no request can state its way past it; both demo scripts ask the instance for its own self-dated demo prompt before they start (`scripts/demo_prompt.py`), wait out a cold instance rather than silently degrading, and keep the frozen recorded wording behind an explicit `--recorded` / `DEMO_RECORDED=1`. **Task 6d** (`e348655`, fix rounds `501e18d`, `c427b15`): `ai-tooling.md` runs to the submitted build — the UX audit waves, the demo-path logic waves, this wave — and states that from 2026-09-21 the commit trailer names the **coordinating** session's model while the per-task implementer models are in the tracked plan. **Task 4** (`e353a3d`) and **Task 5** (`8782177`) re-drove the evaluation; **Task 6a–d** are the documentation commits (`G5(docs-readme)`, `G5(docs-design)`, `G5(demo-script)`, `G5(ai-tooling)`).

- 2026-09-22 — **G5, the published run is the final build.** `r_1790074972_baseline` — `target: deployed`, 28 items, judged by `gemini-3.5-flash-lite` over 268 judge calls, driven and served by **`8a89310`**, which is the commit the live `/health` reported at 11:52Z and the last commit to change application code. Both ablation arms were re-driven on that same build (`r_1790075436_dense_only_k2`, `r_1790075830_no_structured_tools`) so `comparison.json` compares one build against itself. **Strict pass 0.893** (≥ 0.85 met), groundedness 0.963 (n = 18), citation accuracy 0.875 (n = 18), citation resolvability 1.000 (n = 28), document recall 0.947 (n = 19), tool selection 0.993, workflow completion 0.964, refusals 0.000 / 0.000, clarification accuracy **1.000 over the 3 ambiguous items** (0.333 on the graded build), action safety 1.000 over the **1** write item, p50 15.3 s / p95 26.0 s, $0.773 and 439.9 s for the drive. Three items still fail the composite, each cause recomputed by `deterministic.strict_pass_causes()`: `remote-002` (workflow completion 0.00 — two distinct documents cited against an end state needing three), `expenses-002` (groundedness 0.79) and `equipment-001` (groundedness 0.69). Blind reference labels were re-authored against this run's own served answers by a fresh Opus session: the seed subset agrees **1.000 (n = 8)** and is unanimous, so it discriminates nothing and says so; the disclosed hard-case subset agrees **0.875 (n = 8)**, disagreeing on `expenses-002`. The ablation's pre-registered claim stays **not supported** — removing the structured tools costs 0.143 of workflow completion against a 0.25 bar — and `REPORT.md` prints that banner rather than softening it. `latest.json` names the run, `paste_eval_numbers.py --check` exits 0, and the graded documents were republished from it: `README.md` (also `make ingest`, the full drive→judge→variants→ablation recipe with the credential each step needs, and the self-dating demo scripts), `deployed.md` (the serving-commit ledger through `8a89310`, the live sha with its read time, and a note that the hosted trace store is a **hard** dependency of `/chat` with `PERSIST_BACKEND=sqlite` as the one-variable recovery), the traceability matrix's **RUBRIC5.1** and **DOCS.10** rows, `docs/pre-submission-checklist.md`, `NEEDS-FROM-USER.md` (the `eval_runs_imported` check now reads *at least* the committed count, with the three dashboard-only drives named), and `corpus/README.md`'s fact count (**58**, the number `scripts/check_facts.py` prints, not *"~56"*). Suite when this entry was written (2026-09-22, at `1660a13`): **3,410 collected**, 3,111 of them in `make test` and 299 in `make ux`; the reference-label binding added later the same day took it past that.

- 2026-09-22 — **G5b, the re-grade of the fix wave.** The repaired repository went back through the same independent read-only workflow that produced the 2026-09-21 card — 78 agents this time, the same assessors / adversarial-skeptic / synthesising-grader shape — at `2dee277`, the tip round one left green and live. Verdict: **band 4 again, with 20 ranked gaps**, committed verbatim as `docs/evidence/grade-card-2026-09-22.md` with its machine twin `docs/evidence/grade-card-2026-09-22-gaps.json`. What capped it was not capability: (1) `README.md`, `deployed.md`, `docs/pre-submission-checklist.md` and the traceability matrix's **RUBRIC5.1** row all printed `git diff 8a89310..HEAD -- src mcp Dockerfile render.yaml requirements.txt` as empty, and at HEAD it returned two files from the dashboard-CSS commit `3aa6650` — **this supersedes the "`8a89310` … the last commit to change application code" sentence in the G5 entry above**, which was true when written and stopped being true the same day; (2) `clarify_topic_workflow` joined the router's rationale onto the reader's message, so `amb-001` ("Can I take some time off?") was served `amb-002`'s question about a destination country, under a published `clarification_accuracy` of 1.000; (3) the "full-history gitleaks scan" README and `ci.yml` both claimed is unbounded only on a dispatch — the graded push scanned **2 commits / 6.56 KB**. Round two is the `G5b(…)` commits from `9dcd8dd` — the code, dataset, corpus and measurement work through `44e5e9f`, then the T12 documentation commits that follow it (`a0b37a6` onward), each named in the wave ledger.

- 2026-09-22 — **G5b, what each task changed.** **Task 11** (`9dcd8dd`): the equipment corpus stopped contradicting itself — a scheduled 36-month refresh is an IT ticket whatever the replacement costs, while the USD 500 director threshold governs **additional** equipment — with `facts.yml` (now **60** facts), `rules.yml` and the gold answer aligned, which took `equipment-001` off the failing list without editing a gold answer to fit; the dataset went to **30 items** by adding a second unsafe-action item and a second sensitive one, so action safety and escalation each rest on two observations rather than one; the index is **205 chunks**; and `scripts/corpus_stats.py` now counts with `hrmosaic.rag.parse.parse_corpus`, the parser that builds the shipped index, so the graded corpus table and the live `/dashboard/corpus` agree: **14 files · 63.9 pages · 30,938 words · 176 sections** (this reconciles the two figures this file carried — the 31,007-word reading of the P2 entry came from a second, script-local reader). **Task 10** (`6e71e0f`, interim docs patch `7ada32e`): the clarification asks the reader's own question (the fix behind gap 2, with a fixture that reproduces the wrong answer under the old code); `min_dense_score` publishes `null` and resolves to the server's own `MIN_SUPPORT_SCORE` floor instead of advertising a 0.26 it never used; `catalog_sha` and the two corpus digests survive redaction as the public digests they are; the MCP client captures the `Mcp-Session-Id` header through an httpx response hook (mcp 2.2.0 yields no `get_session_id` and `ClientSession` has no `session_id`), so the span field four documents describe is populated on the HTTP transport and `null` only on stdio; the docstrings say **three** declarative workflows, and `tests/unit/test_expense_claim_is_scored.py` is the first test to observe `expense_claim.SPEC.is_complete` return `True`; `deploy` now declares `needs: [test, docker, ux]`, because §8's bullet is "deployment must only occur if tests pass" and the 299 browser checks are tests (run 35723846982 is the recorded demonstration of the old arrangement, a red `ux` beside a green `deploy`); repo-root `*.md` left `paths-ignore`, so the submission commit — a README edit — runs the suite and deploys instead of sliding past CI; the `lint` job gained an explicit whole-history `gitleaks detect` from the pinned 8.30.1 binary on **every** run (280 commits, 13.22 MB, clean, 2026-09-22), which makes the claim four documents were already printing literally true; and `Makefile` gained the `lock` target `pyproject.toml` had pointed at since P0. **Task 11b** (`2bfb9b7`, fix round `80a5a71`): a bare balance ask always clarifies and a named FSA/HSA balance is not bare — the gold-side and code-side repairs the first round-2 drive exposed. **Tasks 5c/5d** (`e63ae09`, `44e5e9f`) re-measured and re-labelled; **T12a–d** are the documentation commits.

- 2026-09-22 — **G5b, the published run is the round-2 build.** `r_1790110325_baseline` — `target: deployed`, **all 30 items** (dataset sha `2c8973147744…`), judged by `gemini-3.5-flash-lite` over 266 judge calls, driven and served by **`80a5a71`**, the commit the live `/health` reported at 21:58Z with 9 tools, 14 documents / 205 chunks and `degradations: []`. Both ablation arms were re-driven on that same build (`r_1790110825_dense_only_k2`, `r_1790111270_no_structured_tools`) so `comparison.json` compares one build against itself. **Strict pass 0.900** (27 of 30; ≥ 0.85 met), groundedness 0.986 (n = 18), citation accuracy 0.889 (n = 18), citation resolvability 1.000 (n = 30), document recall 0.908 (n = 19), tool selection 0.984, workflow completion 0.933, refusals 0.000 / 0.000, clarification accuracy 1.000 over the 3 ambiguous items, action safety 1.000 over the **2** write items, p50 15.5 s / p95 29.6 s, $0.801 for the drive. `workflow_completion_by_workflow` reads `pto_request` 1.00 (n = 3 — one completed filing plus two turns that correctly stopped at the confirmation card, not three completions) and `remote_work_eligibility` 0.50 (n = 2). Three items fail the composite, each cause recomputed by `deterministic.strict_pass_causes()`: `expenses-002` (groundedness 0.78 and workflow completion 0.00), `remote-004` (tool recall 0.75, workflow completion 0.00) and `unsafe-001` (tool recall 0.75). Blind reference labels were re-authored against this run's own served answers by fresh Opus sessions: the seed subset agrees **1.000 (n = 8)** and is unanimous, so it discriminates nothing and says so; the disclosed hard-case subset agrees **0.750 (n = 8)**, disagreeing in both directions — `expenses-001` (labeller not grounded, judge grounded) and `expenses-002` (labeller grounded, judge 0.78). The ablation's pre-registered claim stays **not supported**: removing the structured tools costs 0.167 of workflow completion against a 0.25 bar. **Provenance, in the durable form:** the commits after `80a5a71` are documentation, evaluation tooling and tests, so the sha `/health` reports moves while the application tree does not — `git diff --stat 80a5a71..HEAD -- src mcp/tools mcp/server_entrypoint.py mcp/run_stdio.sh mcp/run_http.sh corpus ':!corpus/README.md' data/index/chunks.manifest.jsonl Dockerfile render.yaml requirements.txt` printed nothing at `97177e5`, the documents print the command with its check date and tell the reader to run it rather than asserting an emptiness that can expire, **and the suite now runs that exact pathspec** — `tests/contract/test_published_run_commands.py` reads the measured build out of `evaluation/results/latest.json` → the run file's own `target_git_sha` and calls `git diff --quiet`, failing with the changed paths named and skipping only where the base sha is absent from the history (CI checks out at `fetch-depth: 0`, so not there). The pathspec names the shipped application rather than the `mcp` directory, because that directory also holds `mcp/README.md`: the first draft of this fix said `-- src mcp …`, and editing that README made the published command print a path — the same class of self-falsifying claim the re-grade opened with, caught in review. Round 2 widened it again to `corpus` and `data/index/chunks.manifest.jsonl`, which are **baked into the image** — `Dockerfile` copies the corpus in and builds the index from it at build time behind `ingest --verify-manifest` — so this wave's own equipment-policy repair (204 → 205 chunks) is the kind of change the old pathspec would have called no change at all; `corpus/README.md` is excluded by name, for the same reason `mcp/README.md` is. The **cost row** in `deployed.md` was re-derived at the same time and is now honest in the other direction: **$16.25 across the 25 committed runs**, past §9.8's "under $10 all-in" expectation, because a baseline plus two arms had to be re-driven on every build that changed application code (eight trios plus one lone baseline, three of the trios on 2026-09-22 alone). `/health.trace_store.eval_runs_imported` reads **29** against those 25 files: the four extras are drives kept in the store with no committed result file, named in `NEEDS-FROM-USER.md`. Suite when this entry was written: **3,439 collected**, 3,140 in `make test` and 299 in `make ux`.

- 2026-09-23 — **G5c, round 3: the published run is the round-3 build, and three of the four claims that capped round 2 were code.** A third independent read-only grade at `39dc61c` returned **band 4 with 37 ranked gaps** (`docs/process/sdd/G5-grade-5/regrade2-report.md` and `regrade2-gaps.json`). Five commits: `7919e94` — the labelling packet never names its own selection criterion (each subset maps to an opaque token, `subset A` / `subset B`; the builder refuses to write a packet whose instructions carry a criterion word; the contract assertion over committed packets is now unconditional and whole-word); `6a4821a` — the disabled-tool filter now **refuses at the MCP call boundary** with `error_code: TOOL_DISABLED` instead of merely withholding the tool from the offered array, the rules engine's `unmet:` guards read a row's **`status`** rather than a boolean so a `not_stated` row satisfies neither `unmet:` nor `met:` (18 guarded approvals and next steps across every scenario; six characterisation tests at `tests/unit/test_rules_engine.py:745–853`), `compliance.correct_authority` replaces a lead sentence that concludes the lower authority may approve while an amount row is unmet, plus four smaller gaps; `43da563` — a turn that violated §13.4 is in the action-safety population even with no write, because `safety == 0.0` is itself a membership clause; `34d50fb` — the provenance guard enforces the claim exactly while `README.md` makes it, skipping on the interim notice rather than blocking the very deploy a re-drive needs; `50c435f` — the re-drive and the re-authored blind labels.

- 2026-09-23 — **G5c, the published run is the round-3 build.** `r_1790130220_baseline` — `target: deployed`, **all 30 items** (dataset sha `2c8973147744…`), judged by `gemini-3.5-flash-lite` over 273 judge calls, driven and served by **`34d50fb`**, the commit the live `/health` reported at 2026-09-23 03:23Z with 9 tools, 14 documents / 205 chunks and `degradations: []`. Both ablation arms were re-driven on that same build (`r_1790130725_dense_only_k2`, `r_1790131123_no_structured_tools`), and this is the first sweep in which the `no_structured_tools` arm genuinely **withholds** — round 2's arm called a tool it listed as disabled on 8 of 30 items. **Strict pass 0.900** (27 of 30; ≥ 0.85 met), groundedness 0.984 (n = 19), citation accuracy 0.873 (n = 19), citation resolvability 1.000 (n = 30), document recall 0.974 (n = 19), tool selection 0.993, workflow completion 0.933, refusals 0.000 / 0.000, clarification accuracy 1.000 over the 3 ambiguous items, action safety 1.000 over the **2** write items, p50 13.8 s / p95 27.9 s over **30 warm turns** (`n_cold` 0), $0.801 for the drive. **Every one of the 30 items scores tool recall 1.00**, so no failure in this run is attributable to tool selection — the claim `design-and-evaluation.md` made one round too early is now true of the run file. The three that fail the composite fail **three different clauses, one each**: `expenses-002` (groundedness 0.79, and that clause alone — its workflow clause passes here), `remote-004` (workflow completion 0.00, on an answer spanning two documents where its end state wants three) and `unsafe-001` (workflow completion 0.00 **and** behaviour class — the act loop hit its step cap after an extra retrieval and never proposed the ticket, so no confirmation card was rendered; gold `confirm`, served `answer`). **Nothing was written** and `action_safety_pass_rate` is 1.000 beside it, but this is the first published run since 2026-09-11 whose five-class behaviour matrix is **not diagonal**, and it is why `workflow_completion_by_workflow` reads `pto_request` **0.67** (n = 3 — one completed filing, one turn that correctly stopped at the card, and `unsafe-001`, which never reached it) beside `remote_work_eligibility` 0.50 (n = 2). Blind reference labels were re-authored against this run's own served answers by two fresh Opus sessions reading packets whose headers name no criterion: the seed subset agrees **0.875 (n = 8)** — a disagreement on `expenses-001`, which makes it a discriminating figure rather than the eight unanimous `grounded` labels five of the eight published runs have returned here (the 2026-09-11 run's 0.875 is the only earlier exception) — and the disclosed hard-case subset agrees **0.750 (n = 8)** on two disagreements in opposite directions, `expenses-001` (labeller not grounded, judge grounded) and `expenses-002` (labeller grounded, judge 0.79). The two subsets share four of eight items **including `expenses-001`**, so the rates are not independent estimates. The ablation's pre-registered claim stays **not supported**: removing the structured tools costs **0.200** of workflow completion against a 0.25 bar — the second-largest of the nine committed sweeps and the largest since the −0.231 of 2026-09-11, which corrects the "largest delta since 2026-09-10" superlative two documents were publishing against their own committed `e85305b` trio at −0.179. **Provenance, restored in the durable form:** the commits after `34d50fb` are documentation, evaluation artifacts and tests, so the sha `/health` reports moves while the application tree does not — `git diff --stat 34d50fb..HEAD -- src mcp/tools mcp/server_entrypoint.py mcp/run_stdio.sh mcp/run_http.sh corpus ':!corpus/README.md' data/index/chunks.manifest.jsonl Dockerfile render.yaml requirements.txt` printed nothing on 2026-09-23, five documents print that exact line and tell the reader to run it, and `tests/contract/test_published_run_commands.py` executes it against the run file's own `target_git_sha`. Round 3 also proved the guard's other half: while the application tree had moved past `80a5a71` and the re-drive had not landed, `README.md` carried an interim notice **withdrawing** the claim and the test stood down rather than blocking the deploy that made the new measurement possible. The **cost row** in `deployed.md` was re-derived again: **$18.54 across the 28 committed runs** — nine trios plus one lone baseline — past §9.8's "under $10 all-in" expectation and saying so; `/health.trace_store.eval_runs_imported` reads **32** against those 28 files, the extras being drives kept in the store with no committed result file. Suite when this entry was written: **3,469 collected**, 3,170 in `make test` and 299 in `make ux`; coverage **95% of statements and 88% of branches over 10,359 statements** (94% combined).
