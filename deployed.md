...
(`E1xxx`, default `E1042`) and `admin`. **One gate, not two:** the token opens everything readable,
and the persona gates *writes* only — the three dashboard write controls (`POST
/api/dev/reset-sandbox`, `POST /api/mcp/rediscover`, `POST /api/eval/runs`, the last of which is the
bounded smoke eval) and the privileged `/chat` options, which answer
...
The **observability dashboard is open to anyone holding the access token**, every page and every
`/api/*` read. All of its data is synthetic, so a grader browses freely: follow the tokenized link
and use the `Chat | Dashboard` switch in the masthead, in whichever persona you are.
