...
- **`Secure` behind Render's edge.** Render terminates TLS at its edge and forwards plain http into
  the container, and uvicorn's `ProxyHeadersMiddleware` rewrites the scheme only for a trusted peer
  (`127.0.0.1` by default), which the edge is not. `web/api.py`'s `request_is_https()` therefore
  reads `X-Forwarded-Proto` for the cookie flag, and for nothing else — forging that header only
  makes the forger's own cookie `Secure`. The same edge is what `ACCESS_RATE_LIMIT_PER_MIN` (30)
  keys on: `request.client.host` is the edge's address for every visitor, so on Render the limit
  behaves as **one shared 30/min bucket** across visitors rather than a per-visitor one. It is a
  drive-by-traffic speed bump either way, and it is genuinely per IP locally and under Docker,
  where nothing sits in front of uvicorn. Trusting a client-supplied `X-Forwarded-For` to split the
  buckets would hand any caller an unlimited supply of them, which is strictly worse than one.
