...
**292.1 MB** against a §14.3 budget of 345 MB and a ceiling of 420 MB — **220 MB of headroom**
below the 512 MB limit. Four runs of the gate on 2026-09-10, each on the image built from the
commit whose `git_sha` the run prints, read **292.9**, **291.3**, **292.1** and **292.1** MB, so the
figure is stable to about a megabyte and the assertion is nowhere near its threshold. The reading is `/proc/self/status` `VmRSS` inside the container (a real
...
| Measured container RSS under `docker run -m 512m` | **292.1 MB** (291.3 and 292.9 MB on earlier runs of the same gate; see the memory section above) | 2026-09-10 | `make docker-run-512` |
